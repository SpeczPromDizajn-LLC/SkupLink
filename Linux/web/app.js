(() => {
  ...
  function fmt(n, digits = 0) {
    if (n === null || n === undefined || Number.isNaN(n)) return t("dash");
  ...
  function fmtWorkMode(sec) {
    ...
    if (s <= 0) return t("work_mode_mains");

    const m = Math.floor(s / 60);
    const r = s % 60;

    return t("work_mode_battery", { min: m, sec: r });
  }
  ...
  function isNetworkError(err) {
    ...
    if (msg.indexOf("network request failed") >= 0) return true;

    if (msg.indexOf("load failed") >= 0) return true;

    if (msg.indexOf("fetch") >= 0 && name === "TypeError") return true;

    return false;
  }

  function friendlyError(err) {
    if (isNetworkError(err)) return t("error_network");

    const msg = err && err.message ? String(err.message) : "";

    if (!msg) return t("error_request");

    if (/^failed to fetch$/i.test(msg)) return t("error_network");
  ...
  async function api(path, options = {}) {
    ...
    if (res.status === 401) {
      logout(false);
      throw new Error(
        data && data.error ? data.error : t("login_auth_required"),
      );
    }

    if (!res.ok) {
      throw new Error(
        (data && data.error) || text || t("error_http", { code: res.status }),
      );
    }
  ...
  function setCharge(percent) {
    if (state.lastCharge === percent) return;
  ...
  function phaseName(index, phaseCount) {
    if (phaseCount <= 1) return t("phase_voltage");

    const map = { 1: "phase_r", 2: "phase_s", 3: "phase_t" };
  ...
  function renderPhases(snap) {
    ...
    const outFreq =
      snap.output && snap.output.frequency != null
        ? fmt(snap.output.frequency, 1)
        : t("dash");
    ...
    if (key === state.lastPhasesHtml) return;
  ...
  function formatPhaseVoltages(phases) {
    if (!phases || !phases.length) return t("dash");
  ...
  function clearOverviewMetrics() {
    ...
    if ($("phases-in")) $("phases-in").innerHTML = "";

    if ($("phases-out")) $("phases-out").innerHTML = "";
  }
  ...
  function updateOverview(snap, options) {
    ...
    const title = [ident.manufacturer, ident.model, ident.name]
      .filter(Boolean)
      .join(" · ");
    ...
    setText(
      ...
      !live
        ? title
          ? `${title} (${t("server_unavailable_suffix")})`
          : t("server_unavailable")
        : title || (snmpOk ? t("ups_generic") : t("ups_no_snmp_data")),
    );
    ...
    setText(
      "m-remain",
      (snap.battery || {}).estimated_minutes_remaining != null
        ? t("remain_minutes", {
            min: (snap.battery || {}).estimated_minutes_remaining,
          })
        : t("dash"),
    );
    setText(
      "m-power",
      t("power_watts", { w: fmt((snap.output || {}).total_power_watts, 0) }),
    );
    setText(
      "m-threshold",
      state.threshold != null
        ? t("threshold_percent", { n: state.threshold })
        : t("dash"),
    );
    setText(
      "m-delay",
      state.delaySeconds != null
        ? t("delay_seconds", { n: state.delaySeconds })
        : t("dash"),
    );
  ...
  function phaseColors() {
    return [cssVar("--teal"), cssVar("--amber"), cssVar("--chart-phase-blue")];
  }
  ...
  function seriesFromArrayField(samples, field, phaseCount) {
    ...
    const lastArr = Array.isArray(samples[samples.length - 1][field])
      ? samples[samples.length - 1][field]
      : [];
    let len = Number(phaseCount) || 0;

    if (len < 1) len = lastArr.length;
  ...
  function drawChart(canvas, series, unit) {
    ...
    const chartBg = cssVar("--chart-bg");
    const chartMuted = cssVar("--muted");
    ...
    (series || []).forEach((s) => {
      (s.values || []).forEach((v) => {
        if (v !== null && v !== undefined && !Number.isNaN(v)) all.push(v);
      });
    });
  ...
  async function refreshHistory(force) {
    ...
    const sig = samples.length
      ? `${samples.length}:${samples[samples.length - 1].ts}`
      : "0";

    if (!force && sig === state.lastSampleCount) return;
    ...
    drawChart(
      $("chart-vin"),
      seriesFromArrayField(samples, "input_voltages", state.inputPhaseCount),
      t("unit_v"),
    );
    drawChart(
      $("chart-vout"),
      seriesFromArrayField(samples, "output_voltages", state.outputPhaseCount),
      t("unit_v"),
    );
    drawChart(
      $("chart-load"),
      seriesFromArrayField(samples, "load_percents", state.outputPhaseCount),
      t("unit_percent"),
    );
    drawChart(
      $("chart-batv"),
      seriesFromScalarField(samples, "battery_voltage", cssVar("--chart-battery")),
      t("unit_v"),
    );
  }
  ...
  function fillSettingsForm(s) {
    ...
    fill("set-host", s.snmp_host || "");
    fill("set-snmp-version", s.snmp_version === "2c" ? "2c" : "1");
    fill("set-community", s.snmp_community || "");
    fill("set-threshold", s.shutdown_battery_percent);
    fill("set-delay", s.shutdown_delay_seconds);
  }
  ...
  async function refreshSettings() {
    ...
    setText(
      "m-threshold",
      t("threshold_percent", { n: s.shutdown_battery_percent }),
    );
  ...
  async function refreshUps() {
    try {
      ...
      if (json === state.lastUpsJson && state.serverOnline) return;

      state.lastUpsJson = json;
      updateOverview(snap, { live: true });
    } catch (e) {
      markServerOffline();
      throw e;
    }
  ...
  function applyBootData() {
    ...
    if (boot.settings) {
      ...
      setText(
        "m-threshold",
        t("threshold_percent", { n: boot.settings.shutdown_battery_percent }),
      );
      setText(
        "m-delay",
        t("delay_seconds", { n: boot.settings.shutdown_delay_seconds }),
      );
    }
  ...
  async function tick() {
    ...
    if (
      !state.token ||
      !document.documentElement.classList.contains("sl-session")
    )
      return;
    ...
    try {
      if (tab === "overview" || tab === "settings") {
        ...
        if (tab === "charts") {
          ...
          const phaseChanged =
            prevIn !== state.inputPhaseCount ||
            prevOut !== state.outputPhaseCount;
          const due = Date.now() - state.lastChartAt >= POLL_MS;
  ...
  function bindUi() {
    ...
    $("btn-discover").addEventListener("click", async () => {
      ...
      try {
        ...
        devices.forEach((d) => {
          ...
          if (d.uid) metaParts.push(`UID ${d.uid}`);

          if (d.mac) metaParts.push(d.mac);

          if (d.rev) metaParts.push(`Rev. ${d.rev}`);
    ...
    window.addEventListener("resize", () => {
      ...
      resizeTimer = setTimeout(() => {
        if (activeTab() === "charts") refreshHistory(true).catch(() => {});
      }, 300);
    });
  ...
  async function loadAppVersion() {
    try {
      ...
      document.querySelectorAll("[data-app-version]").forEach((el) => {
        ...
        el.textContent = t("version_label", { n: ver });
        el.hidden = false;
      });
    } catch (_) {}
  }
